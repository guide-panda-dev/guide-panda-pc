...
### v0.9.6 (현재)
액트 레이싱 타이머 추가 (시작/일시정지/리셋, 홈 이동 시 경고 모달)
가이드 마크다운 링크 [텍스트](url) 렌더링 및 외부 URL 열기 지원
링크 버튼에 URL 선택 텍스트 자동 판별 기능 추가
패시브 이미지 편집에 pending 파일 패턴 적용 — 원본 보호
내보내기 시 패시브 이미지(번들 포함) 항상 복사하도록 개선
v3.28 미라지 리그 - 역학 일제 사격 쇠뇌 토템 하이로펀트의 액트 가이드(유저 데이터 형식) 으로 같이 포함
